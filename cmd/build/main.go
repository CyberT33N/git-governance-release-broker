// Command build runs the Broker's source-level Linux delivery quality gates.
package main
...
const linuxBrokerPath = ".build/bin/broker"
...
func run(
	...
	if err := makeDirectory(filepath.Dir(linuxBrokerPath), 0o755); err != nil {
	...
	fmt.Fprintln(stdout, "Broker source-level build completed successfully.")
	return 0
}
...
func linuxBuildSteps() []step {
	return []step{
		{
			name:        "build Linux AMD64 broker",
			environment: []string{"CGO_ENABLED=0", "GOOS=linux", "GOARCH=amd64"},
			executable:  "go",
			arguments:   []string{"build", "-mod=readonly", "-trimpath", "-o", linuxBrokerPath, "./cmd/broker"},
		},
		{
			name:       "record Linux broker module provenance",
			executable: "go",
			arguments:  []string{"version", "-m", linuxBrokerPath},
		},
	}
}
